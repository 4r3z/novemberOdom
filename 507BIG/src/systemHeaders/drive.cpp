...
vex::rotation sideWaysTracker = vex::rotation(PORT2);
...
void trackPosition()
{
    while (1)
    {
        odom.updatePosition((Right.position(deg) * (0.75 / 360.0 * M_PI * 2.75) + Left.position(deg) * (0.75 / 360.0 * M_PI * 2.75)) / 2, sideWaysTracker.position(deg) * (1.2 / 360.0 * M_PI * 2), imu.heading(degrees));
...
void odom_test()
{
    ...
    while (1)
    {
        ...
        Brain.Screen.printAt(5, 100, "SidewaysTracker: %f", sideWaysTracker.position(degrees) * (1.2 / 360.0 * M_PI * 2.75));
    }
}
